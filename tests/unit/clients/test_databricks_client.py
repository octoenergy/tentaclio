...
from tentaclio.clients.databricks_client import (
    ...
    get_param_from_url_query_or_env_var,
)


def test_build_odbc_connection_string():
    output = build_odbc_connection_string(DRIVER="simba", PORT=443, UID="token")
...
def test_get_param_from_url_query_or_env_var_when_no_env_var_or_url_value(caplog):
    ...
    assert output == ""
    assert (
        "TENTACLIO__KEY not found in env vars and url_key not found in url query"
        in caplog.text
    )
